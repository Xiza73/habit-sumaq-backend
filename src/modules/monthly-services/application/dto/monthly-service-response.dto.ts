...
import type { LinkedDebtSummary } from '../services/linked-debts-gatherer';

/** One entry in `MonthlyServiceResponseDto.linkedDebts`. Swagger-friendly shape. */
export class LinkedDebtResponseDto implements LinkedDebtSummary {
  @ApiProperty({ format: 'uuid', description: 'DebtLoan id — use with POST /debts/:id/settle.' })
  id: string;

  @ApiProperty({ example: 'Ana', description: 'Reference tal cual se guardó (sin normalizar).' })
  reference: string;
...
export class MonthlyServiceResponseDto {
  ...
  @ApiProperty({
    type: [LinkedDebtResponseDto],
    description:
      'Préstamos PENDING generados por pagos compartidos de este servicio ' +
      '(`sourceMonthlyServicePaymentId`). `[]` si el servicio no tiene préstamos vinculados. ' +
      'Se popula en TODOS los endpoints que devuelven este DTO.',
  })
   ...
   * @param linkedDebts PENDING LOANs generated by this service's shared payments. Pass `[]` if
   *   not gathered (defensive default — every call site should gather it via `LinkedDebtsGatherer`).
   */
  static fromDomain(
