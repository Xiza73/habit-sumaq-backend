...
describe('DebtLoanRepositoryImpl', () => {
  ...
  describe('findPendingByReferenceCurrencyType — FIFO ordering for the amount-based settle', () => {
    ...
    it('filters by userId + PENDING + normalized reference + currency + type', async () => {
      const qb = buildQb([]);
      ...
      expect(qb.where).toHaveBeenCalledWith('dl.userId = :userId', { userId: 'user-1' });
      expect(qb.andWhere).toHaveBeenCalledWith(`dl.status = 'PENDING'`);
      expect(qb.andWhere).toHaveBeenCalledWith(
        'LOWER(unaccent(dl.reference)) = LOWER(unaccent(:reference))',
        { reference: 'Juan' },
      );
      expect(qb.andWhere).toHaveBeenCalledWith('dl.currency = :currency', {
        currency: Currency.PEN,
      });
      expect(qb.andWhere).toHaveBeenCalledWith('dl.type = :type', { type: DebtLoanType.DEBT });
    });

    it('orders oldest-first: date ASC, then createdAt ASC, then id ASC (deterministic FIFO)', async () => {
      const qb = buildQb([]);
      ormRepo.createQueryBuilder.mockReturnValue(qb);
      ...
      // Without a stable ORDER BY the FIFO distribution would be
      // nondeterministic (heap order). date ASC drives the "oldest-first"
      // rule; createdAt then id break ties for same-day rows.
      expect(qb.orderBy).toHaveBeenCalledWith('dl.date', 'ASC');
      expect(qb.addOrderBy).toHaveBeenNthCalledWith(1, 'dl.createdAt', 'ASC');
      expect(qb.addOrderBy).toHaveBeenNthCalledWith(2, 'dl.id', 'ASC');
    });

    it('maps ORM rows through toDomain', async () => {
      const qb = buildQb([buildOrmRow({ id: 'debt-x', type: DebtLoanType.DEBT })]);
      ormRepo.createQueryBuilder.mockReturnValue(qb);

      const result = await repo.findPendingByReferenceCurrencyType(
        'user-1',
        'Juan',
        Currency.PEN,
        DebtLoanType.DEBT,
