...
describe('alert-dismiss-policy', () => {
  ...
  // in lockstep.
  it('per-day: service-due-today, habits-midday', () => {
  ...
  it('persistent: service-overdue, budget-overspent, chore-overdue', () => {
    expect(dismissPolicyFor(AlertType.SERVICE_OVERDUE)).toBe('persistent');
    expect(dismissPolicyFor(AlertType.BUDGET_OVERSPENT)).toBe('persistent');
  ...
  it('isDismissable mirrors `policy === per-day`', () => {
    ...
    expect(isDismissable(AlertType.BUDGET_OVERSPENT)).toBe(false);
  });
